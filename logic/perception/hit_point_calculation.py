...
def calc_hit(ray: np.ndarray, convex_hull: ConvexHull, epsilon=1e-6):
    '''
    ...
    left_side = (A @ np.outer(ray, alpha)).T
    constrained = np.array([(left_side[i] - b <= epsilon).all() for i in range(len(alpha))])
    intersection_point = np.ones_like(ray) * np.inf
    intersection_point_norm = np.inf
    for a in alpha[constrained]:
        intersection_candidate = ray * a
        intersection_candidate_norm = np.linalg.norm(intersection_candidate)
        if intersection_candidate_norm < intersection_point_norm:
